(() => {
    ...
    const colorPicker = {
        ...
        resizeCanvas() {
            ...
            const rect = this.svCanvas.parentElement.getBoundingClientRect();
            const w = rect.width || 280;
            this.svCanvas.width = w;
            this.svCanvas.height = 200;
            this.drawSv();
        },
        drawSv() {
            const ctx = this.svCtx, w = this.svCanvas.width, h = this.svCanvas.height;
            ctx.clearRect(0,0,w,h);
            ctx.fillStyle = `hsl(${this.hue},100%,50%)`;
            ctx.fillRect(0,0,w,h);
            const whiteGrad = ctx.createLinearGradient(0,0,w,0);
            whiteGrad.addColorStop(0,'white'); whiteGrad.addColorStop(1,'transparent');
        ...
        hsvToRgb(h,s,v) {
            let r,g,b; const i=Math.floor(h/60), f=h/60-i;
        ...
        syncPreview() {
            if (this.targetPreview && this.targetInput) {
                const val = this.targetInput.value.trim();
                if (/^#[0-9a-fA-F]{3,8}$/.test(val) || /^rgb/.test(val) || /^hsl/.test(val) || val==='transparent')
                    this.targetPreview.style.backgroundColor = val;
                else this.targetPreview.style.backgroundColor = '#ccc';
            }
        },
        open(previewEl, inputEl, independent = false) {
            this.targetPreview = previewEl;
            ...
            this.drawSv();
            this.updateHueThumb();
            this.currentInput.value = this.getCurrentColor();
        ...
        rgbToHsv(r,g,b) {
            ...
            let h=0,s,v=max; const d=max-min; s=max===0?0:d/max;
            if(d!==0) {
                switch(max) {
    ...
    function bindColorPreview(preview, input) {
        if (!preview || !input) return;
        preview.addEventListener('click', e => { e.stopPropagation(); colorPicker.open(preview, input); });
    }
    bindColorPreview($('replaceColorPreview'), replaceInput);
    bindColorPreview($('bgColorPreview'), bgColor);
    ...
    strokeOpacityInput.addEventListener('input', updateBubblePreview); fillColorInput.addEventListener('input', () => { syncColorPreview(fillColorInput, fillColorPreview, fillColorPicker); updateBubblePreview(); }); fillOpacityInput.addEventListener('input', updateBubblePreview); bgColor.addEventListener('input', () => syncColorPreview(bgColor, $('bgColorPreview'), $('bgColorPicker'))); replaceInput.addEventListener('input', () => syncColorPreview(replaceInput, $('replaceColorPreview'), $('replaceColorPicker')));
