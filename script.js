...
const DEFAULT_COMPRESS_URL = 'https://svg.wxeditor.com/tool/svg-compress';
    ...
    const replaceInput = $('replaceInput'), replaceBtn = $('replaceBtn');
    ...
    const textFill = $('textFill'), textContent = $('textContent'), textFont = $('textFont');
    const strokeWidth = $('strokeWidth');
    const bgColor = $('bgColor'), applyBgColorBtn = $('applyBgColorBtn');
    const applyBubbleBtn = $('applyBubbleBtn'), exportBubbleBtn = $('exportBubbleBtn');
...
function updateCompressLinkStatus() {
    ...
    function updateLinkStatus() {
        ...
        try { const p = new URL(url); linkEl.textContent = p.hostname.replace(/^www\./, ''); linkEl.title = url; } catch { linkEl.textContent = url.substring(0,28)+'…'; linkEl.title = url; }
    }
    ...
    function showToast(msg, type='success', dur=3000) {
        ...
        const timer = setTimeout(() => { toast.classList.add('toast-out'); setTimeout(() => toast.remove(),300); }, dur);
        toast.addEventListener('click', () => { clearTimeout(timer); toast.classList.add('toast-out'); setTimeout(() => toast.remove(),300); });
    }
    ...
    function extractColors(text) {
        ...
        if (!matches) return [];
        const seen = new Set();
        return matches.filter(c => { const u=c.toUpperCase(); if(seen.has(u)) return false; seen.add(u); return true; });
    }
    ...
    function customLink() {
        ...
        if($('toSvgModal').classList.contains('active')){
            ...
            iframe.src = trimmed; $('toSvgLoading').classList.remove('hidden');
    ...
    function readFileAndPopulate(file) {
        ...
        reader.onerror = function(){
            uploadZone.classList.remove('loading'); fileInput.value='';
            showToast('❌ 文件读取失败','error'); setInputMode('code');
        };
    ...
    imageUploadZone.addEventListener('drop', e => {
        ...
        if(currentInputMode==='image' && e.dataTransfer.files[0]) handleImageFile(e.dataTransfer.files[0]);
    });
    ...
    function applyBackgroundColor() {
        ...
        handleExtract(); updateBubblePreview();
        showToast(`✅ 画布背景已设置为 ${targetColor}`, 'success');
    }

    function updateBubblePreview() {
        const code = input.value.trim();
        if (!code || !/<svg\b/i.test(code)) { bubblePreviewBox.innerHTML = '<span style="color:#94a3b8;">无有效SVG</span>'; return; }
        const parser = new DOMParser();
        const doc = parser.parseFromString(code, 'image/svg+xml');
        ...
        paths.forEach(path => {
            ...
            if (currentStroke && !/^(#|rgb|rgba|none|transparent|url)/i.test(currentStroke)) path.setAttribute('stroke', '#000000');
        });
        ...
        if (textEl) {
            ...
            textEl.setAttribute('fill', '#000000'); textEl.textContent = textContent.value;
        }
        const serializer = new XMLSerializer();
    ...
    function openBubbleSection() {
        ...
        if (textInfo) {
            ...
            textFill.value = textInfo.fill; textContent.value = '927'; textFont.value = textInfo.fontFamily;
        } else {
            textX.value = '550'; textY.value = '630'; textSize.value = '400';
            textFill.value = '字'; textContent.value = '927'; textFont.value = 'Roboto Condensed';
        }
        strokeWidth.value = '6';
        bgColor.value = (() => {
            try {
                ...
                let x=0,y=0,w=1153,h=1024;
                if(vb){ const p=vb.trim().split(/\s+/); if(p.length===4){ x=parseFloat(p[0]);y=parseFloat(p[1]);w=parseFloat(p[2]);h=parseFloat(p[3]); } }
                const rects = svg.querySelectorAll('rect');
                for(let r of rects) if(Math.abs(parseFloat(r.getAttribute('x')||0)-x)<1 && Math.abs(parseFloat(r.getAttribute('y')||0)-y)<1 && Math.abs(parseFloat(r.getAttribute('width')||0)-w)<1 && Math.abs(parseFloat(r.getAttribute('height')||0)-h)<1) return r.getAttribute('fill')||'transparent';
                const paths = svg.querySelectorAll('path');
                for(let p of paths) {
                    const d = p.getAttribute('d');
                    if(d && /^M\s*([\d.]+)\s+([\d.]+)\s*h\s*([\d.]+)\s*v\s*([\d.]+)\s*H\s*([\d.]+)\s*z$/i.test(d.trim())) {
                        const m = d.trim().match(/^M\s*([\d.]+)\s+([\d.]+)\s*h\s*([\d.]+)\s*v\s*([\d.]+)\s*H\s*([\d.]+)\s*z$/i);
                        if(m && Math.abs(parseFloat(m[1])-x)<1 && Math.abs(parseFloat(m[2])-y)<1 && Math.abs(parseFloat(m[3])-w)<1 && Math.abs(parseFloat(m[4])-h)<1) return p.getAttribute('fill')||'transparent';
                    }
                }
            } catch(e){}
            return 'transparent';
    ...
    function exportBubbleFormat() {
        ...
        if (textEl) {
            textEl.setAttribute('x', textX.value); textEl.setAttribute('y', textY.value);
            textEl.setAttribute('font-size', textSize.value); textEl.setAttribute('fill', textFill.value);
            textEl.setAttribute('font-family', textFont.value); textEl.textContent = '数量';
        }
        const serializer = new XMLSerializer();
        const exportSvg = serializer.serializeToString(doc.documentElement);
        bubbleCodeOutput.textContent = exportSvg;
        bubbleCodeOutput.classList.add('show');
        copyText(exportSvg, '✅ 气泡格式已复制到剪贴板');
    }
    ...
    function convertToReadingFormat() {
        ...
        if (imageEl) {
            imageEl.setAttribute('x', minX); imageEl.setAttribute('y', minY);
            imageEl.setAttribute('width', width); imageEl.setAttribute('height', height);
        }
        ...
        const serializer = new XMLSerializer();
        input.value = serializer.serializeToString(svg);
        handleExtract(); openBubbleSection();
    ...
    exportBubbleBtn.addEventListener('click', exportBubbleFormat);
    [textX, textY, textSize, textFill, textContent, textFont, strokeWidth].forEach(el => { el.addEventListener('input', updateBubblePreview); });
    ...
    function onDragStart(e) {
        ...
        isDragging = true; $('drawerContent').classList.add('dragging'); $('drawerContent').style.transition='none';
    ...
    function onDragEnd() {
        if(!isDragging) return;
        isDragging = false; $('drawerContent').classList.remove('dragging'); document.body.style.userSelect='';
        ...
        $('drawerContent').style.height = getHalfHeight()+'px'; setUIMode(false, true);
    }
    ...
    updateLinkStatus();
    updateLinkStatus();
updateCompressLinkStatus();   // 新增
    input.value = `<svg viewBox="0 0 1153 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="200" height="200">
